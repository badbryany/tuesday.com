...
if (isset($_POST["e_id"]) && isset($_POST["action"])) {
  ...
  if ($_SESSION["uname"] != $row["owner"]) {
    ...
    switch ($_POST["action"]) {
      case 'delete':
        $sql = "DELETE FROM events WHERE e_id=:eventID AND owner=:owner;";
        $sth = $db->prepare($sql);
      break;
      case 'save':
        $sql = "UPDATE events SET title=:newTitle, type=:newType, tags=:newTags, begin=:newBegin, end=:newEnd WHERE e_id=:event_ID AND owner=:owner;";
        $sth = $db->prepare($sql);
